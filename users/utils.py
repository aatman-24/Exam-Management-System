...
class ActivationMailFormMixin:
    ...
    def set_mail_sent(self, value):
        raise TypeError('Cannnot set mail_sent attributes.')
    ...
    def _send_mail(self, request, user, **kwargs):
        ...
        try:
            number_sent = send_mail(**mail_kwargs)  #number_sent will be 0 or 1.
        except Exception as err:
            self.log_mail_error(error=err, **mail_kwargs)
    ...
    def send_mail(self, user, **kwargs):
        ...
        if request is None:
            ...
            logger.warning('send mail called without request.\nTrackback:\n{}'.format(''.join(tb)))
